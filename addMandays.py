...
class AddMandays:
    def __init__(self, window, contractEnd, vendorName, stationName):
        ...
        mgr_label.grid(row=7, column=0, padx=(50, 10), pady=10)
        ...
        tech_label.grid(row=5, column=0, padx=(50, 10), pady=10)

        # MANDAYS ENTRY

        # MANAGER
        mgr_wd_entry = Entry(self.window, textvariable=self.mgr_wd)
        mgr_wd_entry.grid(row=7, column=1, padx=(50, 10), pady=10)

        mgr_cl_entry = Entry(self.window, textvariable=self.mgr_cl)
        mgr_cl_entry.grid(row=7, column=3, padx=(50, 10), pady=10)

        mgr_ft_entry = Entry(self.window, textvariable=self.mgr_ft)
        mgr_ft_entry.grid(row=7, column=4, padx=(50, 10), pady=10)

        mgr_fh_entry = Entry(self.window, textvariable=self.mgr_fh)
        mgr_fh_entry.grid(row=7, column=5, padx=(50, 10), pady=10)

        mgr_nh_entry = Entry(self.window, textvariable=self.mgr_nh)
        mgr_nh_entry.grid(row=7, column=6, padx=(50, 10), pady=10)
        ...
        dsm_wd_entry.grid(row=5, column=1, padx=(50, 10), pady=10)

        dsm_cl_entry = Entry(self.window, textvariable=self.dsm_cl)
        dsm_cl_entry.grid(row=5, column=3, padx=(50, 10), pady=10)

        dsm_ft_entry = Entry(self.window, textvariable=self.dsm_ft)
        dsm_ft_entry.grid(row=5, column=4, padx=(50, 10), pady=10)

        dsm_fh_entry = Entry(self.window, textvariable=self.dsm_fh)
        dsm_fh_entry.grid(row=5, column=5, padx=(50, 10), pady=10)

        dsm_nh_entry = Entry(self.window, textvariable=self.dsm_nh)
        dsm_nh_entry.grid(row=5, column=6, padx=(50, 10), pady=10)
    ...
    def save_mandays(self):
        if (
            ...
            self.dsm_fh.get() and 
            self.dsm_nh.get() and 
            self.tech_wd.get() and 
            self.tech_cl.get() and 
            self.tech_ft.get() and 
            self.tech_fh.get() and 
            self.tech_nh.get() and 
            self.year.get() and 
            self.month.get()
            ):
    ...
    def perform_save_operation(self):
        try:
            ...
            activeSheet['C2'] = self.mgr_cl.get()
            activeSheet['D2'] = self.mgr_ft.get()
            activeSheet['E2'] = self.mgr_fh.get()
            activeSheet['F2'] = self.mgr_nh.get()

            # technician entries
            activeSheet['B3'] = self.tech_wd.get()
            activeSheet['C3'] = self.tech_cl.get()
            activeSheet['D3'] = self.tech_ft.get()
            activeSheet['E3'] = self.tech_fh.get()
            activeSheet['F3'] = self.tech_nh.get()

            # dsm entries
            activeSheet['B4'] = self.dsm_wd.get()
            activeSheet['C4'] = self.dsm_cl.get()
            activeSheet['D4'] = self.dsm_ft.get()
            activeSheet['E4'] = self.dsm_fh.get()
            activeSheet['F4'] = self.dsm_nh.get()
